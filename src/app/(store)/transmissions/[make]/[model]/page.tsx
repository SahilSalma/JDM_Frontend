...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = t('makeModelTransmissionsTitle', { make: matchedMake, model: displayModel });

  return {
    title: `${title} | JDM Tokyo Motorsports`,
    description: `Shop JDM ${matchedMake} ${displayModel} transmissions. Low mileage units sourced directly from Japan, backed by a 30-day warranty. Manual and automatic options available.`,
    openGraph: {
      title,
      description: `Shop JDM ${matchedMake} ${displayModel} transmissions. Low mileage, direct from Japan, with 30-day warranty.`,
    },
  };
}

export default async function TransmissionMakeModelPage({ params }: PageProps) {
  const { make, model } = await params;
  const makes = await getMakes();
  const matchedMake = makes.find((m) => m.toLowerCase() === make.toLowerCase());
  ...
  const breadcrumbs = [
    ...
    { label: matchedMake, href: `/transmissions/${make.toLowerCase()}` },
    { label: displayModel },
  ];
      ...
      <div className="mx-auto max-w-none px-4 py-8 sm:px-6 lg:px-8 xl:px-24 2xl:px-48">
        ...
        <div className="mb-8">
          ...
          <h1 className="font-heading text-3xl font-black uppercase tracking-wide text-foreground sm:text-4xl">
            {t('makeModelTransmissionsTitle', { make: matchedMake, model: displayModel })}
          </h1>
        </div>

        {/* Filtered product grid */}
        <FilteredProductGrid category="transmission" make={matchedMake} model={displayModel} />
      </div>
    </div>
  );
}
